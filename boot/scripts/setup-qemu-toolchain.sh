#!/usr/bin/env bash
# Provisions qemu-system-x86_64 + OVMF UEFI firmware without root.
#
...
# packages (capstone, fdt, pmem, slirp, vdeplug, uring, fuse3, aio) need
# extracting too. Safe to re-run: skips work if already provisioned.
set -euo pipefail
