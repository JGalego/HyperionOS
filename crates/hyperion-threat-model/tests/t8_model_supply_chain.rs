//! docs/17 T8: model supply-chain compromise — content-hash verification
//! blocks a tampered artifact regardless of score, and a canary
//! differential test blocks promotion on score drift.

use hyperion_ai_runtime::{checksum, ModelClass, ModelDescriptor, Precision, QuantizedVariant};
use hyperion_security::{canary_gate_model_promotion, PromotionStatus};

fn signed_descriptor(model_id: u64) -> ModelDescriptor {
    let mut d = ModelDescriptor {
        model_id,
        class: ModelClass::Slm,
        variants: vec![QuantizedVariant {
            precision: Precision::Int8,
            footprint_mb: 256,
            expected_tokens_per_sec: 30.0,
        }],
        checksum: 0,
    };
    d.checksum = checksum(&d);
    d
}

#[test]
fn t8_a_tampered_model_artifact_is_blocked_regardless_of_its_canary_score() {
    let mut poisoned = signed_descriptor(1);
    poisoned.variants[0].footprint_mb += 1; // tampered after the checksum was computed
    let record = canary_gate_model_promotion(&poisoned, 0.95, 0.90, 0.10);

    assert!(!record.checksum_verified);
    assert_eq!(
        record.promotion_status,
        PromotionStatus::Blocked,
        "a poisoned artifact must never be promoted, even with a flattering canary score"
    );
}

#[test]
fn t8_a_legitimate_low_drift_update_is_promoted() {
    let candidate = signed_descriptor(1);
    let record = canary_gate_model_promotion(&candidate, 0.91, 0.90, 0.05);

    assert!(record.checksum_verified);
    assert_eq!(record.promotion_status, PromotionStatus::Promoted);
}
