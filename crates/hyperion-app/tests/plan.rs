...
fn a_clean_answer_becomes_a_buildable_definition() {
    ...
    let definition = from_model_answer(answer, ENGINE, "alice").expect("must parse");
...
fn an_answer_with_nothing_to_run_is_refused() {
    ...
    assert_eq!(
        from_model_answer(answer, ENGINE, "alice").unwrap_err(),
        PlanError::MissingScript
    );
    let blank = r#"{"name":"x","goal":"g","inputs":[],"script":"   "}"#;
    assert_eq!(
        from_model_answer(blank, ENGINE, "alice").unwrap_err(),
        PlanError::MissingScript
    );
}
...
fn an_answer_that_is_not_a_plan_at_all_is_refused_in_plain_words() {
    let err = from_model_answer("I'm afraid I can't help with that.", ENGINE, "alice").unwrap_err();
