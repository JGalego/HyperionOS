...
fn an_undecomposed_goal_compiles_a_renderable_workspace() {
    ...
    let graph = outcome.graph.expect("an ordinary goal always compiles a workspace");
    assert!(!graph.panels.is_empty());
    let tree = outcome.tree.expect("a compiled workspace always carries a tree");
...
fn a_decomposed_plan_renders_one_panel_per_task() {
    ...
    let graph = outcome.graph.expect("a decomposed plan always compiles a workspace");
    assert!(
        graph.panels.len() > 1,
